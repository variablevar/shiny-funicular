"""
GT-Quant Qwen3-8B fine-tuning v0.1 (strong_trend focused, successor to 7B v0.2/v0.3).

Same recipe as train_llm_v03.py (QLoRA r=64, alpha=16, lr 2e-4, 3 epochs,
bs 2x4, seq 2048, 4-bit) but on the Qwen3-8B base with the NON-THINKING
Qwen3 chat template (qwen3-instruct) — this is a JSON-output task, so
reasoning/<think> blocks are disabled.

Output: models/gtquant-8b-qwen3-v0.1/

Run on the RTX 4080 (Ollama must be STOPPED first — see nvidia-smi):
    source venv/bin/activate
    python train_llm_qwen3.py
"""
import torch
...
OUTPUT_DIR = "models/gtquant-8b-qwen3-v0.1"
LORA_R = 64
LORA_ALPHA = 16
LEARNING_RATE = 2e-4
NUM_EPOCHS = 3
